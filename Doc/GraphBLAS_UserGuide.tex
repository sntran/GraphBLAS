...
{\tt NOTE: Version 2.1 is a major update with support for new matrix formats
...
new \verb'GxB_BY_ROW' format.  If you want the default format to be by row,
just compile with \verb'-DBYROW', or add the following after calling
\verb'GrB_init':

    {\footnotesize
    \begin{verbatim}
    GxB_set (GxB_FORMAT, GxB_BY_ROW) ; \end{verbatim}}

This BETA1 version has been fully tested and is ready to use, but the
\verb'GxB_*' extensions added to Version 2.1 may change as feedback from the
GraphBLAS community is received.  These include \verb'GxB_get', \verb'GxB_set',
and \verb'GxB_AxB_METHOD', \verb'GxB_RANGE', \verb'GxB_STRIDE', and
\verb'GxB_BACKWARDS', and their related definitions, described in
...
multithreaded user application.  This support is in Beta status, however,
because of the behavior of \verb'GrB_finalize' discussed below.  User threads
should not operate on the same matrices at the same time, with one exception.
Multiple threads can use the same matrices as inputs to GraphBLAS operations,
but only if they have no pending operations (use \verb'GrB_Matrix_nvals' or
\verb'GrB_wait' first).  User threads cannot simultaneously modify the output
matrix of any GraphBLAS operation.

With multiple user threads, at least thread should call \verb'GrB_init'
before any thread calls any \verb'GrB_*' or \verb'GxB_*' function.

When the user application is finished, {\bf all} user application threads that
called any GraphBLAS operation must call \verb'GrB_finalize', to free up
thread-local workspace allocated internally.  This behavior is a deviation from
the GraphBLAS API and may be modified in the future.

\begin{spec}
{\bf SPEC:} Requiring all user threads in a multi-threaded user application to
call \verb'GrB_finalize' is specific to SuiteSparse:GraphBLAS and is a
deviation from the {\em GraphBLAS C API Specification}.
\end{spec}
...
\verb'GrB_wait' in this case.  When the user application is finished, {\bf all}
user threads must call \verb'GrB_finalize', to free up thread-local workspace
...
it via the \verb'k' parameter of type \verb'void *'.  The user-defined function
should return \verb'false' if \verb'x' is equal the value \verb'(*k)', and
false otherwise.
...
operator is type-generic, \verb'xtype' is returned as \verb'GrB_NULL'.  This is
not an error condition, but simply indicates that the \verb'GxB_SelectOp' is
type-generic.
...
in the \verb'Demo/' folder of SuiteSparse:GraphBLAS use \verb'GrB_mxv' instead,
because in Version 2.0.3 and earlier, SuiteSparse:GraphBLAS only supported a
data structure that stored its matrices by column.  The methods are equivalent;
they just use a matrix \verb'A' that is the transpose of the other.

SuiteSparse:GraphBLAS still stores its sparse matrices by column, by default,
just like MATLAB.  However, it can also be instructed to store any selected
matrices, or all matrices, by row instead, so that \verb'w=A(i,:)' (via
\verb'GrB_Col_extract' with a transposed descriptor) is very fast.  This allows
the BFS example in GraphBLAS API to be done efficiently.  The change in data
format has no effect on the result, just the time and memory usage.  To use a
row-oriented format by default, the following can be done in a user application
that tends to access its matrices by row.
...
\verb'GxB_FORMAT', all matrices will be stored by row.  Alternatively,
SuiteSparse:GraphBLAS can be compiled with \verb'-DBYROW', which changes the
default format to \verb'GxB_BY_ROW', with no calls to any \verb'GxB_*'
function.
...
The default format is by column, just like MATLAB.  This is given by the
predefined value \verb'GxB_FORMAT_DEFAULT', which is equal to \verb'GxB_BY_COL'
if default compile-time options are used.  To change the default at compile
time to \verb'GxB_BY_ROW', compile the SuiteSparse:GraphBLAS library with
\verb'-DBYROW'.  This changes \verb'GxB_FORMAT_DEFAULT' to \verb'GxB_BY_ROW'.
The default hypersparsity ratio is 0.0625 (1/16), but this value may change
in the future.
...
That is, by default, all new matrices are held by column in CSC format, unless
\verb'-DBYROW' is used at compile time, in which case the default is to store
all new matrices by row in CSR format.  If a matrix has fewer than $n/16$
...
all future matrices to be in non-hypersparse CSR when created, use:
...
This changes the matrix \verb'A' so that it is stored by column, and it is
...
SuiteSparse:GraphBLAS is not yet multi-threaded, but it is thread-safe if the
user application threads do not operate on the same matrices at the same time.
Output matrices and vectors used by different threads must be different, and
input matrices and vectors can be safely used only if any pending computations
on them have finished, via \verb'GrB_wait' or the per-matrix methods,
\verb'GrB_*_nvals', \verb'GrB_*_extractElement', \verb'GrB_*_extractTuples',
...
blocking mode determined by \verb'GrB_init'.  Specific format and hypersparsity
parameters of each matrix are specific to that matrix and can be independently
changed.

% \newpage
%===============================================================================
\subsection{{\sf GxB\_set:} set a global option}
%===============================================================================
...
and the format of future matrices to \verb'GxB_BY_ROW'. 
...
%===============================================================================
\subsection{{\sf GxB\_set:} set a matrix option}
%===============================================================================
...
ratio to 0.2, and the format of \verb'GxB_BY_ROW', for a particular matrix
...
% \newpage
%===============================================================================
\subsection{{\sf GxB\_set:} set a {\sf GrB\_Descriptor} value}
%===============================================================================
...
consistent with the other usages of this generic function.  The \verb'field'
parameter can be \verb'GrB_OUTP', \verb'GrB_MASK', \verb'GrB_INP0', or
\verb'GrB_INP1'.  Future versions will allow for different types for the third
parameter.  Refer to Sections~\ref{descriptor_set}~and~\ref{desc_set} for
details.

%===============================================================================
\subsection{{\sf GxB\_get:} retrieve a global option}
%===============================================================================
\label{gxbget}
...
This usage of \verb'GxB_get' retrieves the value of a global option.
The \verb'field' parameter can be \verb'GxB_HYPER' or \verb'GxB_FORMAT'.
For example:
...
% \newpage
%===============================================================================
\subsection{{\sf GxB\_get:} retrieve a matrix option}
%===============================================================================
...
%===============================================================================
\subsection{{\sf GxB\_get:} retrieve a {\sf GrB\_Descriptor} value}
%===============================================================================
...
This usage is identical to \verb'GxB_Desc_get', just with a name
that is consistent with the other usages of this generic function.
The \verb'field' parameter can be \verb'GrB_OUTP', \verb'GrB_MASK',
\verb'GrB_INP0', \verb'GrB_INP1' or \verb'GxB_AxB_METHOD'.  Refer to
Section~\ref{desc_get} for details.
...
\item To specify a strided range of indices with a non-negative stride,
    ...
    the same effect as passing in the the array \verb'I' of size 3, with
...
for one entry \verb'B(j,0)=1', then \verb'C=A*B' is the same as extracting column
\verb'A(:,j)'.  This is very fast if \verb'A' is stored by column but slow if
\verb'A' is stored by row.  If \verb'A' is a sparse row with a single entry
\verb'A(0,i)=1', then \verb'C=A*B' is the same as extracting row \verb'B(i,:)'.
This is fast if \verb'B' is stored by row but slow if \verb'B' is stored by
column.
...
\item If computing \verb"C<M> = A'*B'":   the matrix \verb'B' is
    explicitly tranposed and then rule (2) below is applied.
...
Note that if all matrices are CSR format, \verb'C<M>=A*B' does not explicitly
...
performance.  A different \verb'swap_rule' would allow for the output matrix to
be transposed (with \verb"C=(B*A)'" being computed); a user descriptor may be
added in the future to allow for this selection.  The user application can also
do this by computing \verb"C'=B*A", and then transposing the result via
\verb'GrB_transpose' to obtain \verb"C=(B*A)'=A'*B'".  This can be faster in
some cases, depending on the size and sparsity of the three or four matrices
(\verb'C', \verb'A', \verb'B', and \verb'M' if present).
...
If the \verb'GxB_FORMAT' is \verb'GxB_BY_COL',
\verb'GrB_vxm' with its default descriptor can
...
repeatedly with very sparse vectors \verb'u', it can be faster to
use a \verb'GxB_FORMAT' of \verb'GxB_BY_ROW'.
...
\verb'GrB_mxv' instead of \verb'GrB_vxm', since the graph is assumed to be
symmetric and the multiplier (\verb'AND') is commutative, and since the default
format in SuiteSparse:GraphBLAS is \verb'GxB_BY_COL'.  For the BFS algorithm in
the {\em GraphBLAS C API Specification}, which uses \verb'GrB_vxm', best
performance is obtained in SuiteSparse:GraphBLAS if the format of the \verb'A'
matrix is \verb'GxB_BY_ROW'.
...
If \verb'A' is not transposed: it is fastest if the format of \verb'A' is \verb'GxB_BY_COL'.
The opposite is true if \verb'A' is transposed.
...
If \verb'A' is not transposed: if \verb'|I|' is small, then it is fastest if the format of \verb'C' is \verb'GxB_BY_ROW';
                               if \verb'|J|' is small, then it is fastest if the format of \verb'C' is \verb'GxB_BY_COL'.
The opposite is true if \verb'A' is transposed.
...
\verb'GxB_Col_subassign' is much faster than \verb'GxB_Row_subassign' if the format of \verb'C' is \verb'GxB_BY_COL'.
\verb'GxB_Row_subassign' is much faster than \verb'GxB_Col_subassign' if the format of \verb'C' is \verb'GxB_BY_ROW'.
...
\paragraph{\bf Performance considerations:} % C(i,J) = u'
\verb'GrB_Col_assign' is much faster than \verb'GrB_Row_assign' if the format of \verb'C' is \verb'GxB_BY_COL'.
\verb'GrB_Row_assign' is much faster than \verb'GrB_Col_assign' if the format of \verb'C' is \verb'GxB_BY_ROW'.
...
Assume the matrices are all stored in non-hypersparse form, by column
(\verb'GxB_BY_COL').
...
\verb'C' has a least some dense columns (thus the $\log n$ terms).  If these
...
hypersparse.  Let $\bar{n}$ be the number of non-empty columns, where $\bar{n}<n$, and
sometimes $\bar{n} << n$.  The $\log n$ terms remain since they reflect the binary
search inside a column, but the term $n$ by itself is replaced by $\bar{n}$ or $\bar{n}
\log \bar{n}$.  The term $a$ is replaced by $a \log \bar{n}$.  The total time for a
sequence of $t$ updates to a hypersparse matrix $C$ takes
$O (t \left [a \log \bar{n} + s \log n \right] + (\bar{n} + c + st \log st))$ time.  This
assumes that \verb'C' could include one or more dense columns, with $n$
entries.  If instead the number of entries in each column of \verb'C' is
bounded by a constant, the $\log n$ term becomes a constant.
If \verb'A' is also hypersparse, the value $a$ is removed from the
expression, replaced with its number of non-empty columns, $\bar{a}$.  Suppose
also $t$ is a constant, and $s$ is $O(\bar{a})$.  Suppose the pattern of
\verb'C' changes.  Suppose the number of entries in any one column
of \verb'C' is bounded by $\bar{n}$, and likewise for \verb'A'.
Then the time for the non-hypersparse case simplifies to:
...
column in \verb'C'.  As a result, SuiteSparse:GraphBLAS supports both
...
formulation.  Thus, the ${\bf C \langle L \rangle} = {\bf U}^{\sf T}{\bf L}$ method
...
SandiaDot & \verb"sum(sum((U'*L).*L))"
    & ${\bf C \langle L \rangle} = {\bf U}^{\sf T}{\bf L}$ (dot product) \\
...
its matrices in compressed sparse row form.  SuiteSparse:GraphBLAS (by default)
and MATLAB both store their matrices in compressed sparse column form, so the
Sandia method is identical to \verb'(U*U).*U' in MATLAB and ${\bf C \langle U
\rangle} = {\bf UU}$ in GraphBLAS.  The SandiaDot and SandiaL methods do not
appear in \cite{WolfDeveciBerryHammondRajamanickam17}, but they are named this
way because they are simple extensions of the Sandia method.
...
implementation in SuiteSparse:GraphBLAS has the same performance and memory
requirements as the MATLAB version \verb'nnz(A*E==2)/3'.  That is, both are
very slow.
...
the two SuiteSparse:GraphBLAS methods, on a MacBook Pro (Retina, 13inch, Late
2013), 2.8 Ghz Intel Core i7, 16 GB RAM, OSX 10.11.6, MATLAB 2017a, with the
clang 8.0.0 compiler, using SuiteSparse:GraphBLAS Version 1.1.2.  Only a single
core was used for these results.  In addition, the matrix \verb'L=tril(A)'
and/or \verb'U=triu(A)' are used as-is without any reordering.  The run times
include the time to construct \verb'L' or \verb'U'.  MATLAB failed on one
matrix because \verb'U*U' is too large.  For the first set of matrices, the
outer product formulation (${\bf C \langle U \rangle} = {\bf UU}$) is always
faster than the dot product formulation, but this is not the case for the
second set.
...
multiply in KokkosKernels.  The Kokkos results were done on an Intel Xeon
Haswell (E5-2698v3, 2.3GHz), with 512 GB RAM, 32 cores and 2 hyperthreads per
core, using the Intel \verb'icc' 17.1 compiler.  Unlike the other three
methods, \verb'L' is sorted by decreasing row degree, which improves the
performance.  The Kokkos time includes the time taken to do the sort.  The run
time listed is the best time obtained from several runs with 1 to 32 threads.

Comparing SuiteSparse:GraphBLAS and Kokkos is difficult since these results
were obtained on different machines.  Also, the results in
...
are with a reordered \verb'L' in Kokkos, but not in SuiteSparse:GraphBLAS.
Wolf et al.  \cite{WolfDeveciBerryHammondRajamanickam17} state that reordering
\verb'L' improves the run time.  However, with these many caveats, the last
column lists the speedup of Kokkos over the SuiteSparse:GraphBLAS outer-product
formulation.  Since the Kokkos method is parallel these preliminary comparisons
indicate that the sequential performance of SuiteSparse:GraphBLAS is
...
& \multicolumn{2}{|c|}{${ \bf C \langle U \rangle} = {\bf L}^{\sf T}{\bf U}$}
...
in SuiteSparse:GraphBLAS is very simple:
...
${ \bf C \langle U \rangle} = {\bf L}^{\sf T}{\bf U}$
in SuiteSparse:GraphBLAS is similar:
...
For more discussion on triangle counting and $k$-truss algorithms,
and additional results, see \cite{Davis18b}.
...
By default, SuiteSparse:GraphBLAS stores its matrices by column, using
the \verb'GxB_BY_COL' format.  You can change the default at compile 
time to \verb'GxB_BY_ROW' using the \verb'-DBYROW' flag when compiling.
The advantage to compiling with \verb'-DBYROW' is that a program that
uses GraphBLAS with no SuiteSparse extensions can be written that allows
all matrices to be stored by row, instead of using the default
format by column.  For example:

    {\small
    \begin{verbatim}
    CFLAGS=-DBYROW cmake ..  \end{verbatim} }

The user application can also use \verb'GxB_get' to query the global option
(see also Section~\ref{gxbget}):
...
These options can also be combined.  For example, to use the \verb'gcc'
compiler and to change the default format \verb'GxB_FORMAT_DEFAULT'
to \verb'GxB_BY_ROW', use the following \verb'cmake' command:

    {\small
    \begin{verbatim}
    CC=gcc CFLAGS=-DBYROW cmake ..  \end{verbatim} }
...
I would like to thank John Gilbert (UC Santa Barbara) for many helpful
discussions on GraphBLAS, and for our decades-long conversation on sparse
...
SuiteSparse:GraphBLAS would have been very difficult to implement and test.
