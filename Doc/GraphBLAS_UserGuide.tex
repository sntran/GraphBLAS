...
\begin{verbatim}
This is a BETA release.  The version has passed all tests,
and is ready to use, but it has significant new extensions
to the API (compile-time user-defined objects, in particular).
Sections 8 and 9 are new.  I would like to get feedback on
their design before posting this as the definitive Version 2.2.0.
\end{verbatim}
...
user-defined function $z=f(i,j,m,n,x,\mbox{thunk})$.  The type may be built-in or
...
it via the \verb'thunk' parameter of type \verb'void *'.  The user-defined function
should return \verb'false' if \verb'x' is equal the value \verb'(*thunk)', and
false otherwise.
...
%===============================================================================
\subsection{{\sf GxB\_SelectOp\_fprint:} Print a {\sf GrB\_SelectOp}}
%===============================================================================
...
zombies} in the output report.  Most of the rest of the report is
self-explanatory. 
...
\begin{verbatim}
        ...
        // Part 2: declartions of user-defined variables, and executable
...
once (Part 2).  For example, \verb'User/Example/my_scale.m4' in
...
conflict with internal variables, functions, and macros.  Objects defined by
\verb'GxB_*_define' and user-defined static inline functions and macros (via
\verb'#define') in the \verb'User/*.m4' files must avoid using names with
reserved prefixes \verb'GrB', \verb'GxB', \verb'GXB', \verb'GB', and
\verb'ASSERT'.  User-defined macros have additional restrictions.  A
user-defined macro is safe if it has a name that is all upper case of length 3
characters or more and avoids the reserved prefixes.  For best practice,
however, all user-defined macros should use a unique prefix such as \verb'MY_',
\verb'USER_', or the name of the user application (say \verb'RealCoolApp_').
This will ensure that no name conflicts will occur.
...
just the name of the object.  Since they are \verb'm4' macros, no space can
appear between the macro name \verb'GxB_*_define' and the subsequent left
parenthesis.  Unlike their \verb'GrB_*_new' counterparts, the six
\verb'GxB_*_define' macros do not return an error code.  Any errors will be
...
\end{verbatim} } \end{mdframed}
    ...
    must be a built-in type (\verb'GrB_BOOL', \verb'GrB_FP64', etc) or a type
    defined with \verb'GxB_Type_define'.
...
\end{verbatim} } \end{mdframed}
    ...
    binary operator defined by \verb'GxB_BinaryOp_define'.  The identity is a
    scalar value (not a pointer, as in \verb'GrB_Monoid_new').  The three types
    of the operator must be the same, but this cannot be checked by this method
    at compile time.  Results are undefined if this condition does not hold.
...
By default, SuiteSparse:GraphBLAS stores its matrices by row, using
the \verb'GxB_BY_ROW' format.  You can change the default at compile 
time to \verb'GxB_BY_COL' using the \verb'-DBYCOL' flag when compiling.
For example:

    {\small
    \begin{verbatim}
    CFLAGS=-DBYCOL cmake ..  \end{verbatim} }
...
to \verb'GxB_BY_COL', use the following \verb'cmake' command:

    {\small
    \begin{verbatim}
    CC=gcc CFLAGS=-DBYCOL cmake ..  \end{verbatim} }
