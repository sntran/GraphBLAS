...
)
{
    ...
    if (nargin > 2)
    {
        ...
        ASSERT_OK (GB_check (op2, "binary op", D0)) ;
    }
    else
    {
        // get a unary op
        ...
        ASSERT_OK (GB_check (op1, "unary op", D0)) ;
    }

    ASSERT_OK (GB_check (op_ztype, "Z type", D0)) ;
    ...
    ASSERT_OK (GB_check (X_type, "X type", D0)) ;
    ...
    if (nargin > 2)
    {
        ...
        ASSERT_OK (GB_check (Y_type, "Y type", D0)) ;
        Y_size = Y_type->size ;
